...
describe('nft-staking', () => {
  ...
  const [rewardsMint,] = anchor.web3.PublicKey.findProgramAddressSync([Buffer.from("rewards"), config.toBuffer()], program.programId);
  const [userAccount,] = anchor.web3.PublicKey.findProgramAddressSync([Buffer.from("user"), provider.publicKey.toBuffer()], program.programId);
  const [stakeAccount,] = anchor.web3.PublicKey.findProgramAddressSync([Buffer.from("stake"), new anchor.web3.PublicKey(nftMint.publicKey as PublicKey).toBuffer(), config.toBuffer()], program.programId);

  it("Mint Collectdion NFT", async() => {
    await createNft(umi, {
      mint: collectionMint,
      name: "GM",
      symbol: "GM",
      uri: "https://tedkim.dev",
  ...
  it("Verify Collectdion NFT", async () => {
  ...
  it("Initialize Config Account", async () => {
    ...
    const tx = await program.methods.initializeConfig(pointsPerStake, maxStake, freezePeriod)
      .accountsPartial({
